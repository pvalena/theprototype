...
touch "${PREFIX}ZOMBIE_DC.txt" "${PREFIX}SKIP.txt" "${PREFIX}FAIL.txt"
...
[[ -r "${PREFIX}ZOMBIE.txt" && -r "${PREFIX}ZOMBIE_DC.txt" && -r "${PREFIX}SKIP.txt" && -r "${PREFIX}FAIL.txt" ]] || abort "Files unreadable"

while read PKG; do
  # Skip already checked
  grep -q "^${PKG}$" "${PREFIX}ZOMBIE_DC.txt" && {
    skip "$PKG" 'ZOMBIE_DC.txt'
    continue
  }
  ...
  # Check users for package == orphan
  USERS="$(
    curl -s "${SRC_FPO_RPMS}${PKG}" \
      | jq -r '.access_users[][]'
  )" || R=1
  [[ $R -eq 0 ]] || {
    skip "$PKG" "curl / jq(USERS)"
    sleep "$LONG_WAIT"
    continue
  }
  [[ -n "$USERS" ]] || fail "$PKG" "\$USERS"
  [[ "$USERS" == 'orphan' ]] && {
    fail "$PKG" "\$USERS = $USERS"
    continue
  }
  ...
  [[ -z "$BRANCHES" ]] || {
    fail "$PKG" "\$BRANCHES = $BRANCHES"
    continue
  }

  echo "$PKG" >> "${PREFIX}ZOMBIE_DC.txt"
